...
使用 `@mobile-next/mobile-mcp`，一个专为 agent 控制移动端模拟器设计的 MCP server。通过 MCP 协议与 Claude Code 集成，在操作系统层操作 iOS Simulator，与 app 内部实现（Expo Go / development build）无关。
...
在 `~/.claude/claude.json` 全局配置中注册 MCP server：
...
- Android Emulator 同样支持，未来扩展无需额外配置
